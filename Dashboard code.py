...
trend_df = df[(df["INDICATOR_LABEL"] == selected_indicator) &
              (df["REF_AREA_LABEL"] == selected_country)].dropna(subset=["OBS_VALUE"])
if not trend_df.empty:
    fig3 = px.line(trend_df, x="TIME_PERIOD", y="OBS_VALUE",
                   title=f"{selected_indicator} Trend — {selected_country}", markers=True)
...
composition_df = df[(df["INDICATOR_LABEL"] == "Municipal Solid Waste (MSW) Composition") &
                    (df["TIME_PERIOD"] == selected_year) &
                    (df["WASTE_PRODUCT_LABEL"] != "Not applicable")].dropna(subset=["OBS_VALUE"])
